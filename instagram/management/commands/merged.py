...
class Command(BaseCommand):
    ...
    def run_tasks(self):
        ...
        # print(33)
        accounts_created = 0
        user_cred_list=list(user_detail_local.objects.filter(updated = False,status = 'ACTIVE'))
        # print(44)
        ...
        for i_user in range(len(user_cred_list)):
            random_user_for_login = user_detail_local.objects.filter(updated = False,status = 'ACTIVE').first()
            try:
                for i in range(3):
                    ports = list(
                        filter(
