...
class Command(BaseCommand):
    ...
    def run_tasks(self, required_accounts):
        ...
        elegible_time = datetime.datetime.now() - datetime.timedelta(days=7)
        elegible_time = timezone.utc.localize(elegible_time)
        all_random_user = list(user_detail_local.objects.filter(created_at__gte=elegible_time,status="ACTIVE"))
        random_user_count = 0
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    # if self.Create :
                    # if random_user_count < len(all_random_user):
                    new_user= all_random_user[random_user_count]
                    random_user_count += 1
                    if random_user_count < len(all_random_user):
                        random_user_count = 0
                    insta_login,login_issue = tb.login_account(new_user.username,new_user.password)
                # new_user = tb.create_account()
                    ...
                    if self.Follow_to_follow :
                        tb.deny_permission1(deny_count=True)
                        followed_user = 0
                        follow_loop = 100 - new_user.following
                        ...
                        for i in range(follow_loop):
                            for i1 in range(3):
                                ...
                                if follow_user.username == new_user.username:continue 
                                ...
                                #     if following == 'Follow':
                                    ...
                                    follow_user.save()
                                    if following < 0:
                                        new_user.following += 1
                                        new_user.save()
                                    print(f'\n\n{follow_user.followers}follow_user.followers\n\n')
                                    print(f'\n\n{new_user.following}user_cred.following\n\n')
                                    break



                    if self.Engagement and False:
                    ...
                    if self.Profile:
                        ...
                        random_sleep(10,15)
                        print('\n\n')
                        print(self.Connect_vpn,'self.Connect_vpn')
                        print(self.Create,'self.Create')
                        print(self.Random,'self.Random')
                        print(self.Ontarget,'self.Ontarget')
                        print(self.Engagement,'self.Engagement')
                        print(self.Profile,'self.Profile')
                        print(self.Process,'self.Process')
                        print(self.continue_process,'self.continue_process')
                        print('\n\n')
